        ...
        <div id="div1" class="table-responsive">
            <table ng-table="tableList" show-filter="false" class="table table-condensed table-bordered table-hover" style="white-space: nowrap;">
                <tr ng-repeat="(k,v) in $data" class="pointer" ng-click="read(v.id)">
                    ...
                    <td title="'Stok'" filter="{stok: 'text'}" sortable="'stok'">{{v.stok}}</td>
                </tr>
            </table>
        </div>
    </div>
    ...
    <div class="ibox-content frmEntry">
        <div class="row">
            <div class="col-sm-4">
                ...
                <input type="text" ng-model="h.jenis" class="form-control input-sm" >
                ...
                <textarea ng-model="h.keterangan" class="form-control input-sm" rows="4"></textarea>
                <label title="harga">Harga</label>
                <input type="text" ng-model="h.harga" class="form-control input-sm" awnum="default">
                <label title="stok">Stok</label>
                <input type="text" ng-model="h.stok" class="form-control input-sm" awnum="default">
            </div>
        </div>
    </div>
</div>
...
app.controller('mainCtrl', ['$scope', '$http', 'NgTableParams', 'SfService', 'FileUploader', function($scope, $http, NgTableParams, SfService, FileUploader) {
    SfService.setUrl("<?=base_url()?>products");
